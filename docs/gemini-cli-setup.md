# Using ae-skills with Gemini CLI
...
gemini skills install /path/to/ae-skills/skills/
```

**Install for a specific workspace only:**

```bash
gemini skills install /path/to/ae-skills/skills/ --scope workspace
```
...
cat /path/to/ae-skills/skills/incremental-implementation/SKILL.md > GEMINI.md
echo -e "\n---\n" >> GEMINI.md
cat /path/to/ae-skills/skills/code-review-and-quality/SKILL.md >> GEMINI.md
```
...
To replicate the `ae-skills` experience from other tools, you can configure a `SessionStart` hook that reminds you of the available skills or loads a meta-skill.
